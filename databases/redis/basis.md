...
Redis 缓存失效策略有：定时删除策略，惰性删除策略，定期删除策略。  
> 定时删除策略  
> 在设置 key 的过期时间的同时，为该 key 创建一个定时器，让定时器在 key 的过期时间来临时，对 key 进行删除。它的优点是保证内存尽快释放；缺点是如果 key 过多，删除这些 key 会占用很多 CPU 时间， 而且每个 key 创建一个定时器，性能影响严重。  
> 
> 惰性删除策略  
> key 过期的时候不删除，每次从数据库获取 key 的时候去检查是否过期，若过期，则删除，返回 null。它的优点是 CPU 时间占用比较少；缺点是如果 key 很长时间没有被获取， 将不会被删除，可能造成内存泄露。  
> 
> 定期删除策略  
