...
- 由于 Windows 平台文件名不区分大小写，所以文件名应一律使用小写；不同单词之间用下划线分词，不要使用驼峰式命名；如果是测试文件，可以以 _test.go 结尾。
- 变量、函数、方法使用驼峰命名法。
...
- 函数名还是使用 驼峰命名法；当你不需要在包外访问，请使用小写字母开头
- 接口使用驼峰命名法，可以用 type alias 来定义，大写开头的 type 给包外访问

2. 代码格式
- 包引入了三种类型的包，标准库包，程序内部包，第三方包依次分块导入。
- 使用 gofmt 工具格式化代码。
- 使用 4 个空格代替制表符缩进。
- 使用 UTF-8 编码。
...
4. 静态分析  
go vet 工具可以帮我们静态分析我们的源码存在的各种问题，例如多余的代码，提前 return 的逻辑，struct 的 tag 是否符合标准等。
```bash
