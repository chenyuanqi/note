...
目的：单元测试是用来验证单个组件（例如函数、方法或类）的行为。它们帮助您确保代码按预期工作，并且在将来的修改中不会引入错误。
...
每个测试用例是一个函数，以 Test 开头，例如 func TestMyFunction(t *testing.T) {...}。  
使用 t.Errorf 或 t.Fatalf 来报告错误或失败。  
对于想要测试的每个函数，考虑各种可能的输入和预期的输出。使用断言来验证函数的实际输出是否符合预期。考虑边界条件和异常情况。  
```go
...
**模拟依赖**  
对于有外部依赖的函数（例如数据库或网络服务），考虑使用模拟（mocking）来隔离测试。  
Go 社区有多个模拟库，例如 gomock 和 testify。  


### Gin + Gorm 单元测试
纵观业务逻辑代码，写好单元测试的关键是业务逻辑拆分足够细的函数式、目录结构分层职责明确。  
